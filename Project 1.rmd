...
ipa_ale_comp %>%
    ggplot(aes(x = IBU, y = ABV, color = style_strict)) +
    geom_point() +
    xlab("IBU") +
    ylab("ABV") +
...
{
    trainIndices <- sample(seq(1, ipa_ale_len, 1), split_perc * ipa_ale_len)
...
state_df_perc %>%
    ...
    ggtitle("State High ABV/IBU Ale Representation") +
    theme_minimal()
...
state_df_perc %>%
    ...
    geom_bar(stat = "identity", fill = "#44a149") +
    xlab("State") +
    ylab("Difference from Percent Total") +
    ggtitle("State High ABV/IBU IPA Representation") +
    theme_minimal()
```
