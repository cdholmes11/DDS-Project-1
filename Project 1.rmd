...
#knn
```{r}
ipa_ale_comp <- brew_beers_new %>% filter(style_strict == "Ale" | style_strict == "IPA") %>%
filter(!is.na(ABV) & !is.na(IBU))
```
...
{
  trainIndices = sample(seq(1,ipa_ale_len,1), splitPerc * ipa_ale_len)
  train = ipa_ale_comp[trainIndices,]
  test = ipa_ale_comp[-trainIndices,]
  for(i in 1:numks)
  {
    classifications = knn(train[c(3,4)],test[c(3,4)],train$style_strict, prob = TRUE, k = i)
...
MeanAcc = as.data.frame(colMeans(masterAcc))
colnames(MeanAcc) = c("masterAcc")
bestknn = which.max(MeanAcc$masterAcc)
max(MeanAcc)

classifications = knn(train[,3:4],test[,3:4],train$style_strict, prob = TRUE, k = bestknn)
table(classifications,test$style_strict)
CM = confusionMatrix(table(classifications,test$style_strict))
CM
```